      ...
      <div className="card mb-4">
        <div className="flex gap-4">
          ...
          <div>
            ...
            <p className="text-muted text-sm">{shortAddress(wallet)}</p>
            <div className="flex gap-2 mt-2">
              <Stars score={user?.reputation_score || 0} />
              <span className="text-sm text-muted">({user?.total_jobs_completed || 0} jobs completed)</span>
            </div>
          </div>
        </div>
        ...
        {stats && (
          <div className="grid-4 mt-4">
            <MiniStat label="Earned" value={`${stats.totalEarned.toFixed(2)} USDC`} />
      ...
      <form onSubmit={handleSave} className="card flex-col gap-4">
        ...
        <div className="grid-2">
          ...
          <div className="form-group">
            ...
            <input className="input" type="number" min="0" value={hourlyRate} onChange={(e) => setHourlyRate(e.target.value)} placeholder="50" />
          </div>
        </div>

        <button type="submit" className="btn btn-primary" disabled={saving}>
          {saving ? <><div className="spinner spinner-sm" /> Saving...</> : "Save Profile"}
        </button>
      </form>
    </div>
  );
